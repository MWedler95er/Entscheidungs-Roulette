...
Bevor du das Spiel zum ersten Mal startest, benötigst du:

1. **Docker Umgebung**
   - Installierte Docker Engine (Windows, macOS oder Linux)
   - Docker Compose v2 (ist bei aktuellen Docker-Desktop-Installationen dabei)

2. **Quellcode**
   - Das Repository / den Projektordner lokal, z.B.:
     ```bash
     git clone <DEIN-REPO>
     cd Entscheidungs-Roulette
     ```

3. **(Nur Linux/macOS) Rechte für das Start-Script setzen**
   - Einmalig ausführen:
     ```bash
     chmod +x run_game.sh
     ```

4. **Keine weiteren lokalen Abhängigkeiten**
   - Du brauchst **kein** lokales Python, keine virtuelle Umgebung.
   - Alle Python-Abhängigkeiten werden im Docker-Image über `requirements.txt` installiert.

Danach kannst du das Spiel direkt mit dem Script starten (siehe Abschnitt „Spiel starten“).
